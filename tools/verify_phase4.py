"""verify_phase4.py：Phase 4 串口日志断言（Task 4.12 总验收）。
对 qemu_capture.py 产生的 serial log 逐项核对：
- APP_VERSION=<run/expected_version.txt> 出现且无 ERROR 行；
- STATE 序列：GCS 1→3→4→5（AUTO_START 进游戏）且 GCS 5→7（Level 1 全通）；
- SPAWN 序列：含 boss 坦克 id=0A（Level1BossTank）与 EndofLevelCheck id=46。
--mode autostart：AUTO_START+AUTOSCROLL 验证构建（Level 1 全通证据链）。
退出码 0=全过；失败项逐条打印。零第三方依赖。
用法：python tools/verify_phase4.py [--serial run_logs/xxx_serial.log] --mode autostart
"""
import argparse
import re
import sys
...
def main():
    ...
    check('GCS 5->7 (Level 1 全通：boss 收尾 → 过关)', ('GCS=5', 'GCS=7') in gcs_pairs)

    spawns = re.findall(r'SPAWN id=([0-9A-F]{2})', log)
    check('SPAWN id=0A (Level 1 boss tank)', '0A' in spawns)
    check('SPAWN id=46 (EndofLevelCheck)', '46' in spawns)

    print(f'state seq: {" -> ".join(seq)}')
    print(f'spawn tail: {" ".join(spawns[-16:])}')
    if fails:
